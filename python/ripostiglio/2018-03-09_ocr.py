...
''' 
    ...
    # § To-do:
        - method lowercase_after_punctuation()
            - should also prevent the first word of a paragraph from being capitalized
            - should first create a list of all <rs> names already existing in g.xml
                - then mark "[punctuation] than_name" as "[punctuation] <rs>than_name</rs>"
                - then run the rest of the script
                - at the end, some cases like "sunt. paulus est..." will still exist
                    - they'll be handled 'by hand' during my collation on the MS
        - ...then, re-run this script on input.txt
            - but before, comment out the "newly scanned" paragraphs from g.xml to avoid xml:id duplicates
        - the script should mark with <rs> proper names at the beginning of the line (after <lb>)
...
def properNames (properNamesInputXmlFile):
    ''' Parse an xml file and return a set (not list) of names
        marked with <rs> in that file. All names are in lowercase.
        '''
...
class ocr:
    ...
    def old_lowercase_after_punctuation (self):
        ''' Transform uppercase initials to lowercase after punctuation such as full stop.
            E.g.: "est. Ergo..." to "est. ergo..." (because they mostly are no proper names).
            '''
        for l in self.lines:
            i = self.lines.index(l)
            callback = lambda pat: pat.group(1) + pat.group(2).lower()
    ...
    def xml_export (self, outputXmlFile):
        ...
        os.system('cp %s %s' % (outputXmlFile, '_'.join([outputXmlFile, datetime, 'backup.xml']))) # Create backup of old file
