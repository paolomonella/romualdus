...
def myReplaceAll (myGraph, myAlpha, myElement, wholeWord=False):
    ''' If wholeWord=False (default), this function replaces the simple string myGraph (e.g.: "q9")
    ...
    if wholeWord:       
        if myGraph[-1] in [p for p in punctuation]:
            myGraph = r'\b%s\b%s\B' % (myGraph[:-1], myGraph[-1])   # Result is like   r'\bqd\b-\B'. Man, this was hard to code!
            # This 'if' block is necessary b/c "\bqd-\b" does not match " qd- ", since "-" is a regex word-delimiter
            # If I'll use abbrev. marks that are word-delimiters, I'll have to add them to this list. But as of now
            # I'm choosing to make things easier and only use abbrev. marks that are no regex word-delimiters.
        else:
...
def genericBaseReplaceAll (myRow, myElement):
    ''' This applies to ToS 'grapheme' cells in the format '[aeiouy]3', where the brackets include
    ...
    myAbbrMark = firstcellmatch.group(2)     # group(2) is like "3" (the abbreviation mark). It includes one char only
    secondcellmatch = re.match('\[.*\](.+)',   myRow[1])
    myExpansion = secondcellmatch.group(1)  # group(1) is like "m" (the alphabetic expansion: "u3" becomes "um")
    for e in myElement.findall('.//*'):
